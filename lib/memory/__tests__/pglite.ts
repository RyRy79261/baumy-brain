...
CREATE TABLE baumy_memory_embeddings (id uuid PRIMARY KEY DEFAULT gen_random_uuid(), memory_item_id uuid NOT NULL REFERENCES baumy_memory_items(id) ON DELETE CASCADE, model text NOT NULL, embedding vector(384) NOT NULL);
CREATE TABLE baumy_entities (id uuid PRIMARY KEY DEFAULT gen_random_uuid(), group_id text NOT NULL REFERENCES baumy_telegram_chats(chat_id), kind text NOT NULL, canonical_name text NOT NULL, aliases text[], name_embedding vector(384), is_active boolean NOT NULL DEFAULT true, created_at timestamptz NOT NULL DEFAULT now());
