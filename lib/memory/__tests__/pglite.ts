...
CREATE TABLE baumy_telegram_updates (update_id bigint PRIMARY KEY, chat_id text, status text NOT NULL DEFAULT 'received', raw jsonb, received_at timestamptz NOT NULL DEFAULT now());
CREATE TABLE baumy_replies (update_id bigint PRIMARY KEY, sent_at timestamptz NOT NULL DEFAULT now());
...
CREATE TABLE baumy_list_items (id uuid PRIMARY KEY DEFAULT gen_random_uuid(), group_id text NOT NULL REFERENCES baumy_telegram_chats(chat_id), list_name text NOT NULL DEFAULT 'shopping', item text NOT NULL, item_normalized text NOT NULL, added_by text REFERENCES baumy_members(telegram_user_id) ON DELETE SET NULL, checked_by text REFERENCES baumy_members(telegram_user_id) ON DELETE SET NULL, checked_at timestamptz, is_active boolean NOT NULL DEFAULT true, created_at timestamptz NOT NULL DEFAULT now());
CREATE INDEX baumy_list_items_group_open_idx ON baumy_list_items (group_id, is_active);
CREATE UNIQUE INDEX baumy_list_items_open_uq ON baumy_list_items (group_id, list_name, item_normalized) WHERE (is_active AND checked_at IS NULL);
...
CREATE TABLE baumy_house_config (id boolean PRIMARY KEY DEFAULT true, house_group_chat_id text, live_chat_id text, migrated_from_chat_id text, reminder_thread_id bigint, console_thread_id bigint, house_timezone text NOT NULL DEFAULT 'Europe/Berlin', response_policy jsonb NOT NULL DEFAULT '{"global_enabled":true,"categories":{},"confidence_threshold":0.7,"muted_topics":[]}'::jsonb, daily_spend_cap_usd numeric NOT NULL DEFAULT '0.50', secure_key_version smallint NOT NULL DEFAULT 1, updated_at timestamptz NOT NULL DEFAULT now());
